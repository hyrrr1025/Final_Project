...
public class PlatformD : MonoBehaviour
{
    ...
    public GameObject NewobjectD;// 物体 B 的预制件
    public float delay = 5.0f;      // 延迟时间

    private bool isCharacterOnPlatform = false;  // 检测人物是否在平台上
    private bool isSpawning = false;             // 防止多次调用 SpawnAndDropObjectB
    private GameObject spawnedNewobjectD = null;      // 记录生成的唯一 Storm 实例
    ...
    void SpawnAndDropObjectD()
    {
        if (isCharacterOnPlatform && spawnedNewobjectD == null)
        {
            ...
            // 设置生成物体的名称
            spawnedNewobjectD.name = "NewobjectD";  // 这里可以根据需要命名
